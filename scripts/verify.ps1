...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-prism-ops-schedule-detail.ps1

node tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-prism-ops-schedule-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-prism-ops-schedule-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-prism_ops_schedule_guard-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
